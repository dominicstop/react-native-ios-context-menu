...
| 🔤  `shouldEnable`<br>`AggressiveCleanup`<br/><br/>⚛️ `boolean`<br/><br/>✳️ **Default**: `true` | If set to `true` (which it is by default), the custom preview and auxiliary preview are immediately freed from memory once the context menu is closed.<br><br>📝 **Note**: For the immediately cleanup to take effect, the  `lazyPreview` prop also needs to be enabled. |
